...
use Aliyun\OSS\Model\LiveChannelConfig;
...
use Aliyun\OSS\Model\StorageCapacityConfig;
use Aliyun\OSS\Model\WebsiteConfig;
use Aliyun\OSS\Result\AclResult;
use Aliyun\OSS\Result\AppendResult;
use Aliyun\OSS\Result\BodyResult;
use Aliyun\OSS\Result\CallbackResult;
use Aliyun\OSS\Result\CopyObjectResult;
use Aliyun\OSS\Result\DeleteObjectsResult;
use Aliyun\OSS\Result\ExistResult;
use Aliyun\OSS\Result\GetCnameResult;
use Aliyun\OSS\Result\GetCorsResult;
use Aliyun\OSS\Result\GetLifecycleResult;
use Aliyun\OSS\Result\GetLiveChannelHistoryResult;
use Aliyun\OSS\Result\GetLiveChannelInfoResult;
use Aliyun\OSS\Result\GetLiveChannelStatusResult;
use Aliyun\OSS\Result\GetLocationResult;
use Aliyun\OSS\Result\GetLoggingResult;
use Aliyun\OSS\Result\GetRefererResult;
use Aliyun\OSS\Result\GetStorageCapacityResult;
...
use Aliyun\OSS\Result\ListLiveChannelResult;
...
use Aliyun\OSS\Result\PutLiveChannelResult;
use Aliyun\OSS\Result\PutSetDeleteResult;
use Aliyun\OSS\Result\SymlinkResult;
...
class OssClient
{
    ...
    public function deleteObjects($bucket, $objects, $options = null)
    {
        ...
        if (isset($options['quiet'])) {
            ...
            } else if (is_string($options['quiet'])) { // string
    ...
    private function computePartSize($partSize)
    {
        ...
        } else if ($partSize > self::OSS_MAX_PART_SIZE) {
    ...
    private function auth($options)
    {
        ...
        if (isset($options[self::OSS_FILE_UPLOAD])) {
            if (is_resource($options[self::OSS_FILE_UPLOAD])) {
                ...
                if (isset($options[self::OSS_CONTENT_LENGTH])) {
                    $length = $options[self::OSS_CONTENT_LENGTH];
                } else if (isset($options[self::OSS_SEEK_TO]) && isset($length)) {
    ...
    private function authPrecheckObjectEncoding(&$options)
    {
        ...
        try {
            ...
            } else if (OssUtil::checkChar($options[self::OSS_OBJECT], true)) {
    ...
    private function checkEndpoint($endpoint, $isCName)
    {
        ...
        } else if (OssUtil::isIPFormat($ret_endpoint)) {
