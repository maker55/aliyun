...
use Aliyun\Vod\Request\CreateUploadAttachedMediaRequest;
use Aliyun\Vod\Request\CreateUploadImageRequest;
...
class VodClient
{
    ...
    public function __construct($config)
    {
        ...
        $this->apiRegionId = $config['reginid'] ?? 'cn-shanghai';
    ...
    public function uploadLocalM3u8($uploadVideoRequest, $sliceFiles)
    {
        ...
        // 上传到点播的m3u8索引文件会重写，确保分片地址都为相对地址
    ...
    private function initOssClient($uploadAuth, $uploadAddress)
    {
        ...
        $this->ossClient = new OssClient([
            'accesskey'     => $uploadAuth->AccessKeyId,
            'accesssecret'  => $uploadAuth->AccessKeySecret,
            'endpoint'      => $endpoint,
    ...
    {
        $request = new CreateUploadAttachedMediaRequest();
    ...
    public function getVideoInfo($videoId)
    {
        ...
        $request->setVideoId('01c6191df51b46298883a0e27a4280cc');

        $data = $this->vodClient->getAcsResponse($request);
        $endpoint = AliyunVodUtils::convertOssInternal( $this->apiRegionId, $this->ecsRegionId, $this->enableSSL);
        ...
        $this->ossClient->setUseSSL(1);

       dd( $this->ossClient->signUrl('outin-1b41899c1e1f11e98a5100163e1c94a4','sv/3b7246bf-168a8004eef/3b7246bf-168a8004eef.mp4',1550132154-time()));
